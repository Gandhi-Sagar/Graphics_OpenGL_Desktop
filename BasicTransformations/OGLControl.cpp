...
void COGLControl::Render(LPVOID lParam)
{
	...
	if( (tCurrent-m_tLastSecond) >= CLOCKS_PER_SEC)
	{
		m_tLastSecond += CLOCKS_PER_SEC;
...
void COGLControl::SetProjection3D(float fFOV, float fAspectRatio, float fNear, float fFar)
{
	m_mProjection = glm::perspective(fFOV, fAspectRatio, fNear, fFar);
}

glm::mat4* COGLControl::GetProjectionMatrix()
{
...
int		COGLControl::GetFPS()
{
	return m_iFPSCount;
}

bool	COGLControl::SetVerticalSync(bool bEnabled)
{
