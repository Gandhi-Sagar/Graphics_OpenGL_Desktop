...
bool COGLWindow::CreateAppWindow(wstring sWindowName)
{
	...
	if(! m_oOGLControl.InitOpenGL(m_hInstance, &m_hWnd, 3, 1, InitScene, RenderScene, NULL, &m_oOGLControl))
		return false;
...
{
	if(! appMain.InitializeApp(L"BlankWindow_3.3"))
		return 0;
	appMain.RegisterAppClass(hInstance);

	if( !appMain.CreateAppWindow(L"BlankWindow_3.3"))
		return 0;
	appMain.ResetTimer();
	appMain.AppBody();
	appMain.ShutDown();
